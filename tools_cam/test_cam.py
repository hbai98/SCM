...
def main():
    ...
    if VALIDATE_SET:
        ...
        det_correct, loc_correct, cls_correct = val_loc_one_epoch(
            val_loader, model, device)
        res = evaluate((det_correct, loc_correct, cls_correct, num_imgs))
        opt_thred = res['det_optThred_thr_50.00_top-1']
    else:
        opt_thred = -1

    num_imgs = len(test_loader.dataset)
    det_correct, loc_correct, cls_correct = val_loc_one_epoch(test_loader, model, device, )
    eval_results = evaluate((det_correct, loc_correct, cls_correct, num_imgs))
...
def draw_bboxes_images(dataloader, model, opt_thred, device, cfg):
    ...
    with torch.no_grad():
        model.eval()
        for _, (input, gt_labels, bbox, image_names) in tqdm(enumerate(dataloader)):
            ...
            for i in range(len(inputs)):
                ...
                iou = max_iou
                boxed_image = draw_bbox(blend, iou, np.array(
                    gt_bbox).reshape(-1, 4).astype(np.int32), bbox, draw_box=False, draw_txt=False)
...
def val_loc_one_epoch(val_loader, model, device):
    ...
    with torch.no_grad():
        model.eval()
        print('Collect data ... ')
        for _, (input, gt_labels, bbox, _) in tqdm(enumerate(val_loader)):
            # update iteration steps
            gt_labels = gt_labels.to(device)
            input = input.to(device)

            cls_scores, cams = model(input, return_cam=True)
    ...
    return det_correct, loc_correct, cls_correct
