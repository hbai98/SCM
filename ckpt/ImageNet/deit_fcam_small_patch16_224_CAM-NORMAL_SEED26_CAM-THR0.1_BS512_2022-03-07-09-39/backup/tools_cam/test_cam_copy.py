...
def main():
    ...
    if VALIDATE_SET:
        cams, gt_labels, cls_logits, gt_bboxes, image_names = val_loc_one_epoch(val_loader, model, device)
        res = evaluate((cams, gt_labels, cls_logits, gt_bboxes))
        opt_thred = res['det_optThred_thr_50.00_top-1']
    else:
        opt_thred = -1
        
    cams, gt_labels, cls_logits, gt_bboxes, image_names = val_loc_one_epoch(test_loader, model, device, )
    eval_results = evaluate((cams, gt_labels, cls_logits, gt_bboxes))
    ...
    if DRAW:
        ...
        draw_bboxes_images(cams, gt_labels, gt_bboxes, opt_thred, image_names, cfg)

def draw_bboxes_images(dataloader, cams, gt_labels, gt_bboxes, opt_thred, image_names, cfg):
    print('Start Drawing images ...')
    for j, (inputs, _, _, _) in tqdm(enumerate(dataloader)):
        B = inputs.shape[0]
        inputs = input.cpu()
...
def val_loc_one_epoch(val_loader, model, device):
    ...
    with torch.no_grad():
        model.eval()
        print('Collect data ... ')
        for i, (input, target, bbox, image_names_) in tqdm(enumerate(val_loader)):
            ...
            results.append((cams_, target, cls_logits_, gt_bbox, image_names_)) 
    
    cams, gt_labels, cls_logits, gt_bboxes, image_names = zip(*results)
    ...
    return cams, gt_labels, cls_logits, gt_bboxes, image_names
...
def evaluate(
    ...
    preds, gt_labels, cls_scores, gt_bboxes = results
