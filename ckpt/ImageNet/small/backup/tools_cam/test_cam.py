...
def main():
    ...
    if DRAW:
        ...
        draw_bboxes_images(test_loader, model, 0.35, device, cfg)
    ...
    with torch.no_grad():
        ...
        for _, (input, gt_labels, bbox, image_names) in tqdm(enumerate(dataloader)):
            ...
            for i in range(len(inputs)):
                ...
                if len(np.array(gt_bboxes).shape) == 1:
                    gt_bbox = [gt_bbox]
                ...
                boxed_image = draw_bbox(blend, iou, np.array(
                    gt_bbox).reshape(-1, 4).astype(np.int32), boxes_at_thresholds, draw_box=True, draw_txt=True)
